...
export const RoleSwitcher = () => {
  ...
  const getRoleConfig = (roleCode: string) => {
    ...
    switch (code) {
      ...
      default:
        return { icon: <User size={16} />, color: 'text-gray-600', bg: 'bg-gray-100', border: 'border-gray-200', label: roleCode };
    }
  };
              ...
              {availableRoles.map((role) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 flex items-center justify-between rounded-lg mb-1 transition-all ${
                      isActive 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {isActive && <p className="text-[10px] text-gray-500 font-medium">Current Context</p>}
                      </div>
                    </div>
                    {isActive && (
                      <Check size={16} className={config.color} />
                    )}
                  </button>
                );
              })}
